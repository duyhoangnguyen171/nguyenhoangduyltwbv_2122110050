...
//select * from order where status!='0' and ... orderby created_at desc...
//status =1 --> hiện trang người dùng
// =2 --? không hiện
// =0 --> rác
$list = Order::where('status', '!=', 0)
   ->select('status', 'id', 'deliveryname')
                           ...
                           <td>
                              ...
                              <div class="function_style">
                                 <?php if ($item->status == 1) : ?>
                                    <a class="text-success" href="index.php?option=order&cat=status*id= <?= $item->id; ?>">Hiện</a> |
                                 <?php else : ?>
                                    <a class="text-danger" href="index.php?option=order&cat=status*id= <?= $item->id; ?>">Ẩn</a> |
                                 <?php endif; ?>
